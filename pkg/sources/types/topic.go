...
// ParseTopicTag maps a free-form string to a TopicTag when possible.
// It supports a small set of synonyms to avoid duplicating logic in providers.
func ParseTopicTag(s string) (TopicTag, bool) {
    switch normalize(s) {
...
// UniqueTopicTags deduplicates and normalizes a list of strings to TopicTags.
func UniqueTopicTags(values ...string) []TopicTag {
    seen := make(map[TopicTag]bool)
    out := make([]TopicTag, 0)
    for _, v := range values {
        if tag, ok := ParseTopicTag(v); ok {
            if !seen[tag] {
                seen[tag] = true
                out = append(out, tag)
            }
